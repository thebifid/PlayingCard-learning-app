...
//
//  Created by Vasiliy Matveev on 26.05.2020.
...
class PlayingCardView: UIView {
    ...
    private func drawPips(){
        ...
        func createPipString(thatFits pipRect: CGRect) -> NSAttributedString {
            ...
            let probablyOkayPipFontSize = verticalPipRowSpacing / (attemptedPipString.size().height / verticalPipRowSpacing)
            let probablyOkayPipString = centredAttributedString(suit, fontSize: probablyOkayPipFontSize)
            if probablyOkayPipString.size().width > pipRect.size.width / maxHorizontalPipCount {
                return centredAttributedString(suit, fontSize: probablyOkayPipFontSize / (probablyOkayPipString.size().width / (pipRect.size.width / maxHorizontalPipCount)))
            } else {
...
extension PlayingCardView {
    private struct SizeRatio {
        ...
        static let corneeRadiusToBoundsHeight: CGFloat = 0.06
    ...
    private var cornerFontSize: CGFloat {
        return bounds.size.height * SizeRatio.corneeRadiusToBoundsHeight
    }
